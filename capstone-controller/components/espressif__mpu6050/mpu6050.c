...
static esp_err_t mpu6050_write(mpu6050_handle_t sensor, const uint8_t reg_start_addr, const uint8_t *const data_buf, const uint8_t data_len)
{
    ...
    uint8_t * buffer = calloc(1+data_len, sizeof(uint8_t));
    buffer[0] = reg_start_addr;
    memcpy(buffer+1, data_buf, data_len*sizeof(uint8_t));
    ret = i2c_master_transmit(sens->dev, buffer, 1+data_len, 1000 / portTICK_PERIOD_MS);
    free(buffer);
    return ret;
}
...
static esp_err_t mpu6050_read(mpu6050_handle_t sensor, const uint8_t reg_start_addr, uint8_t *const data_buf, const uint8_t data_len)
{
    ...
    ret = i2c_master_transmit_receive(sens->dev, &reg_start_addr, 1, data_buf, data_len, 1000 / portTICK_PERIOD_MS);
    return ret;
}
